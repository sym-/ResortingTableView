...
typedef NS_ENUM(NSInteger, RTSnapshotMeetsEdge){
    RTSnapshotMeetsEdgeTop,
    RTSnapshotMeetsEdgeBottom,
};

@interface ResortTabelView ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation ResortTabelView
...
-(void)awakeFromNib{
    ...
    [self registerNib:[UINib nibWithNibName:NSStringFromClass([ArticleTableViewCell class]) bundle:nil] forCellReuseIdentifier:TextCellIdentifier];
    [self registerNib:[UINib nibWithNibName:NSStringFromClass([ImageTableViewCell class]) bundle:nil] forCellReuseIdentifier:ImageCellIdentifier];
...
-(void)setTableDataSource:(NSMutableArray *)tableDataSource{
    ...
    [self.tableDataSource addObjectsFromArray:tableDataSource];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ResortModel *model = self.tableDataSource[indexPath.row];
    
    switch (model.cellType) {
        case TTTextType:
        {
            ArticleTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:TextCellIdentifier forIndexPath:indexPath];
            cell.model = model;
        ...
        case TTImageType:
        {
            ImageTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ImageCellIdentifier forIndexPath:indexPath];
            cell.model = model;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if ([_relocatedIndexPath isEqual:indexPath]) {
        return 50;
    }
    return 150;
}
...
#pragma mark 排序相关
-(IBAction)longPressAction:(UILongPressGestureRecognizer *)longPress{
...
- (void)updateDataSource{
    ...
    [self.tableDataSource exchangeObjectAtIndex:_originalIndexPath.row withObjectAtIndex:_relocatedIndexPath.row];
...
//    UIGraphicsBeginImageContextWithOptions(inputView.bounds.size, NO, 0);
    UIGraphicsBeginImageContextWithOptions(CGSizeMake(inputView.bounds.size.width,50), NO, 0);
